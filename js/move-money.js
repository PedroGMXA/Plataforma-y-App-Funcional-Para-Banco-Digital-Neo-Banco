...
const moveMoneyModule = {
    ...
    renderHub() {
        // ***** CORRECCIÓN DE ID DE PLANTILLA *****
        // Se usa el ID correcto del HTML: 'move-money-hub-template'
    ...
    renderTransferStep2(contact) {
        ...
        // ***** CORRECCIÓN DE SALDO *****
        // Se calcula el saldo disponible sumando TODAS las tarjetas de débito y digitales.
        const availableBalance = this.globalState.cards
            .filter(card => card.type === 'Débito' || card.type === 'Digital')
    ...
    renderTransferStep3() {
        ...
        const debitCard = cards.find(c => c.type === 'Débito' || c.type === 'Digital');
    ...
    handleContinueToConfirm() {
        ...
        // ***** CORRECCIÓN DE SALDO (VALIDACIÓN) *****
        const availableBalance = this.globalState.cards
            .filter(card => card.type === 'Débito' || card.type === 'Digital')
            .reduce((total, card) => total + (card.balance || 0), 0);

        if (amount <= 0) { this.appController.showModal('El monto debe ser mayor a cero.'); return; }
        if (amount > availableBalance) { this.appController.showModal('No tienes saldo suficiente.'); return; }
